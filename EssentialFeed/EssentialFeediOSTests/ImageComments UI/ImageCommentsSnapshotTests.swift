...
final class ImageCommentsSnapshotTests: XCTestCase {
    func test_feedWithContent() {
        ...
        assert(snapshot: sut.snapshot(for: .iPhone13(style: .light)), named: "COMMENTS_WITH_CONTENT_light")
        assert(snapshot: sut.snapshot(for: .iPhone13(style: .dark)), named: "COMMENTS_WITH_CONTENT_dark")
        assert(snapshot: sut.snapshot(for: .iPhone13(style: .light, contentSize: .extraExtraExtraLarge)), named: "COMMENTS_WITH_CONTENT_light_extraExtraExtraLarge")
    }
    ...
    private func commentsWithContent() -> [CellController] {
        return [
